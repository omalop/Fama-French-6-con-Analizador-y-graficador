...
def apply_indicators(df):
    ...
    # Definimos las condiciones respetando que los overrides en PineScript se aplican al final.
    # Con np.select la PRIMERA condición verdadera empata, así que los overrides van PRIMERO.
    conditions = [
        # Overrides por cruce
        (df['WPR'] < -50) & (df['WPR_Prev'] >= -50), # Cruce bajista (Magenta)
        (df['WPR'] > -50) & (df['WPR_Prev'] <= -50), # Cruce alcista (DeepNavy)

        # Zona Alcista (wpr > -50)
        (df['WPR'] > -50) & (df['WPR_Down']) & (df['Sig_Up']),   # Rojo: Corrección c/ fuerza
        (df['WPR'] > -50) & (df['WPR_Down']) & (df['Sig_Down']), # Amarillo: Sin fuerza
        (df['WPR'] > -50) & (df['WPR_Up']) & (df['Sig_Down']),   # Navy: Retroceso leve (pullback)
        ...
        # Zona Bajista (wpr <= -50)
        (df['WPR'] <= -50) & (df['WPR_Up']) & (df['Sig_Down']),  # Cyan: rebote sin fuerza
        (df['WPR'] <= -50) & (df['WPR_Down']) & (df['Sig_Down']),# Orange: bajista sin fuerza
        (df['WPR'] <= -50) & (df['WPR_Down']) & (df['Sig_Up']) & (df['WPR'] < lower_band), # BrownDark: bajista fuerte
        (df['WPR'] <= -50) & (df['WPR_Down']) & (df['Sig_Up']) & (df['WPR'] >= lower_band),# Purple: fuerza bajista media
        (df['WPR'] <= -50) & (df['WPR_Up']) & (df['Sig_Up'])     # BrownOrange: reinicio impulso bajista
    ]

    choices = [
        'Cruce Bajista (Magenta)',
        'Cruce Alcista (Azul Profundo)',
        'Correccion Fuerte (Rojo)', 
        'Sin Fuerza (Amarillo)', 
        'Pullback (Azul)', 
        'Impulso Fuerte (Verde Osc)', 
        'Impulso Medio (Verde)',
        'Pullback Bajista (Cian)',
        'Bajista Sin Fuerza (Naranja)',
        'Bajista Fuerte (Marron Oscuro)',
        'Fuerza Bajista Media (Morado)',
        'Reinicio Bajista (Marron Naranja)'
    ]

    df['Status_Control'] = np.select(conditions, choices, default='Neutral')
