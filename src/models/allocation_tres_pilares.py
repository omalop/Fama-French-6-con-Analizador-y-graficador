...
if __name__ == "__main__":
    ...
    try:
        inf_anual_str = input("  📈  Inflación anual esperada (ej: 45.0): ").strip()
        inflacion_anual_esperada = float(inf_anual_str)/100.0 if inf_anual_str else 0.45
    except (ValueError, EOFError):
        inflacion_anual_esperada = 0.45
    ...
    print(f"  ✅ Escenario: Confianza {confianza_gobierno}% | Infla Mes {inflacion_mensual_esperada:.1%} | Infla Anual {inflacion_anual_esperada:.1%} | Devalu {devalu_mensual_esperada:.1%}")
    ...
    if not df_global_sel.empty and alloc['RV_Global'] > 0:
        top_sec = seleccionar_por_umbral(
            ...
            umbral_score=UMBRAL_GLOBAL, min_activos=5, max_activos=5,
            aplicar_momentum=True
        )
        print(f"  {'Ticker':<10} {'Sector':<25} {'FF Score':>8}  {'% Capital':>10}")
        print(f"  {'-'*60}")
    ...
    metadata = {
        ...
        "inflacion_anual_esperada": inflacion_anual_esperada,
    ...
    # --- INTEGRACIÓN: MONITOREO DE CARTERA ACTIVA (MÉTODO DOMENEC) ---
    print(f"{SEP}")
    print("📊 MÓDULO DE SEGUIMIENTO DE CARTERA ACTIVA".center(90))
    print(f"{SEP}")
    try:
        from src.models.monitor_posiciones import monitorear_cartera
        monitorear_cartera()
    except Exception as e:
